...
from dspyagent import ChatConsole, Agent
...
class RetrieveAgent(Agent):
    ...
    def forward(
        ...
        self._console.next_speaker(self.name, f"{message}")
        raw_doc = self._search(message)
        # self._console.markdown(raw_doc)
        # print()
        # self._console.delivery(self._name, user_name, raw_doc)
        return {"doc": f"{raw_doc}"}
